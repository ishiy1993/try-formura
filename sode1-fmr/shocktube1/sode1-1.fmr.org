...
begin function (b_vis,u_vis,p_vis) = viscosity((b,u,p),(b_x,u_x,p_x))
c = (gm*p*b)**(1/2)
b_vis = 0.0
u_vis = 0.0
p_vis = if u_x < 0.0 then vis*(c*u_x*h/b - (gm+1)/2 * u_x**2 * h**2) else 0.0
end function

begin function (b_vis,u_vis,p_vis) = viscosity_x((b,u,p),(b_x,u_x,p_x))
c = (gm*p*b)**(1/2)
u_xx = d_xx(u,u_x)
b_vis = 0.0
u_vis = 0.0
p_vis = if u_x < 0.0 then vis*(c*u_x*h/2/b/b*(1+b/p) + c*u_xx*h/b - c*u_x*b_x*h/b/b - (gm+1) * u_x * u_xx * h**2) else 0.0
end function
...
qp' = q' + dt*q_t + dt*dt*q_tt/2 + smoo(qp) + viscosity(qp, qp_x)
qp_x' = q_x' + dt*q_tx + dt*dt*q_ttx/2 + smoo(qp_x) + viscosity_x(qp, qp_x)

qh' = q' + dt*q_t/2 + dt*dt*q_tt/12 + smoo(qh) + viscosity(qh, qh_x)
qh_x' = q_x' + dt*q_tx/2 + dt*dt*q_ttx/12 + smoo(qh_x) + viscosity_x(qh, qh_x)
